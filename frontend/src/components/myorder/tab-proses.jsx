...
export default function TabProses({ loading, setLoading }) {
  const [orders, setOrders] = useState([]);
  const router = useRouter();
  useEffect(() => {
    setLoading(false);
    const fetchData = async () => {
      const response = await axios.get("api/order?status=proses", {
        headers: {
      ...
      {orders.map((order) => (
        <div key={order.id}>
          ...
          <div className="w-full">
            ...
            <Button className="ms-auto block mt-3" disabled>
